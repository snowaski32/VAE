...
class VAEXperiment(pl.LightningModule):
    ...
    def sample_images(self):
        ...
        # if not os.path.exists(os.path.join(self.logger.log_dir ,
        #                                "Reconstructions",
        #                                f"true_{self.logger.name}.png")):
        # vutils.save_image(test_input[0],
        #                 os.path.join(self.logger.log_dir ,
        #                             "Reconstructions",
        #                             f"true_{self.logger.name}_Epoch_{self.current_epoch}.png"),
        #                 normalize=True)
        recons = self.model.generate(test_input, labels=test_label)
        data = torch.cat((recons[0].unsqueeze(0), test_input[0].unsqueeze(0)))
        print(data.shape)
        vutils.save_image(
            data,
            os.path.join(
        ...
        try:
            samples = self.model.sample(4, self.curr_device, labels=test_label)
            vutils.save_image(
                samples.cpu().data,
                os.path.join(
                    self.logger.log_dir,
